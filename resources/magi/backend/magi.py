#!/usr/bin/env python3
"""Magi's dependency-free host backend and CLI. One process per connected host."""
import argparse
...
class Backend:
    def __init__(self, root=None):
        self.root = Path(root or os.environ.get("MAGI_ROOT", str(Path.home() / "Documents" / "Magi"))).expanduser().resolve()
    ...
    def repo_attach(self, workspace, repo, branch="", new_branch="", base="HEAD", **_):
        with self.lock():
            ...
            branch_name = ref(branch or new_branch or ("magi/" + workspace))
    ...
    def install_cli(self):
        folder = self.root / "utils" / "magi"
        folder.mkdir(parents=True, exist_ok=True)
        for filename in ("magi.py", "sess", "SESS-LICENSE"):
...
def main():
    ...
    try:
        if host != "local":
            ...
            bootstrap = "import sys,json,os;from pathlib import Path;p=json.loads(sys.stdin.buffer.raw.readline());r=Path(p['root']).expanduser().resolve();d=r/'utils'/'magi';d.mkdir(parents=True,exist_ok=True);[(d/k).write_text(v) for k,v in p['files'].items()];os.execv(sys.executable,[sys.executable,'-u',str(d/'magi.py'),'--root',str(r),'--rpc'])"
            files = {f: (Path(__file__).parent / f).read_text() for f in ("magi.py", "sess", "SESS-LICENSE")}
