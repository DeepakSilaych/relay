...
Read [AGENTS.md](AGENTS.md), [the style guide](docs/STYLEGUIDE.md), and [the architecture guide](docs/relay/lite-v1.md). Preserve Orca and third-party attribution. Reuse existing UI primitives and design tokens.
...
```sh
pnpm exec tsc --noEmit -p config/tsconfig.relay-app.json
...
Run the relevant scripts in `resources/relay/tests` for UI changes. Always use `ORCA_BACKGROUND_LAUNCH=1`, a disposable `RELAY_ROOT`, and a disposable `RELAY_USER_DATA_PATH`. VM tests must use a dedicated temporary root. Never test against another person's active sessions or bring test windows to the foreground.
...
A maintainer packages Apple Silicon artifacts with `pnpm release:relay:mac`, verifies the bundle signature and packaged behavior, publishes to this repository, and updates the version and DMG SHA-256 in `DeepakSilaych/homebrew-tap`. Keep the application identifier stable. Current builds are ad-hoc signed; automatic installation must remain disabled until signed and notarized updates have been tested.
