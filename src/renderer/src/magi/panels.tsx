...
function readPanels(): Record<Side, Panel> {
  try {
    const saved = JSON.parse(localStorage.getItem('magi.panels') || '{}')
...
export function usePanels() {
  ...
  useEffect(() => {
    localStorage.setItem('magi.panels', JSON.stringify(panels))
  }, [panels])
...
export function WorkspaceHeader({
  ...
  return (
    <header className="magi-drag flex h-10 shrink-0 items-center gap-3 border-b px-4">
      <span
