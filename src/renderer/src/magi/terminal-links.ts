...
export function installTerminalLinks(
  ...
  const resolve = (paths: string[]) =>
    window.magi.request<(OpenFile | null)[]>(host, 'resolve_links', { workspace, terminal, paths })
  term.options.linkHandler = {
    ...
    activate(event, uri) {
      ...
      if (/^https?:\/\//.test(uri)) {
        void window.magi.openExternal(uri).catch(report)
